...
class LiftNode(Node):
    def __init__(self, lift_name):
        ...
        self.clientSecret = (
            "592a91ee0c89005933d6cef74ff0bb66cd9953d5f19c4e57d4ea6b6e7810c4c3"
        )
        ...
        self.koneAdaptorGalen = koneAdaptor(
            self.clientID, self.clientSecret, self.buildingID
        )
        ...
        self.reset_liftstate_ws_timer = self.create_timer(
            5.0, self.reset_liftstate_ws_CallBack
        )
        self.liftstate_timer = self.create_timer(1.0, self.liftStatePublisherCallBack)

        threadKoneLiftstateWS = Thread(
            target=self.liftstate_websocket,
            name="koneliftstate",
            args=("koneliftstate",),
        )
    ...
    def reset_liftstate_ws_CallBack(self):
        ...
        last_active_timestamp = (
            self.koneAdaptorGalen.last_active_timestamp_for_liftstate_ws
        )
    ...
    def liftRequestCallBack(self, msg):
        ...
        if (
            self.prev_rmf_lift_request[current_lift_index].destination_floor
            == msg.destination_floor
            and self.prev_rmf_lift_request[current_lift_index].lift_name
            == msg.lift_name
            and self.prev_rmf_lift_request[current_lift_index].session_id
            == msg.session_id
        ):
            req_time_elapsed = (
                msg.request_time.sec
                - self.prev_rmf_lift_request[current_lift_index].request_time.sec
            )
        ...
        current_source_floor = self.koneAdaptorGalen.current_liftstate_list[
            current_lift_index
        ].current_floor
        ...
        if current_dest_floor != current_source_floor:
            ...
            self.get_logger().info(
                "Sending lift command paylod now. Lift: %s, Destination Floor: %s"
            ...
            self.koneAdaptorGalen.liftDestinationCall(
                current_source_floor, current_dest_floor, msg.lift_name
            )
            ...
            self.get_logger().info(
                "Lift: %s , Destination floor == Current floor. Skipping!"
                % msg.lift_name
            )
